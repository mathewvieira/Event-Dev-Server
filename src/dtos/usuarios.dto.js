...
const insertableFieldsUsuario = ['id_comunidade', 'email', 'senha', 'funcao', 'usuario_root', 'ativo']

const updatableFieldsUsuario = ['id_comunidade', 'email', 'senha', 'funcao', 'usuario_root', 'ativo']
...
const validateDataUsuario = (usuario) => {
  ...
  if (!usuario.senha || typeof usuario.senha !== 'string' || usuario.senha.length < 6)
  ...
  if (
    usuario.id_comunidade !== null &&
    usuario.id_comunidade !== undefined &&
    typeof usuario.id_comunidade !== 'bigint' &&
    typeof usuario.id_comunidade !== 'number'
  ) {
    errors.push('id_comunidade deve ser um número ou nulo.')
  }

  if (usuario.usuario_root !== undefined && typeof usuario.usuario_root !== 'boolean')
    errors.push('usuario_root deve ser um valor booleano.')
  if (usuario.ativo !== undefined && typeof usuario.ativo !== 'boolean')
